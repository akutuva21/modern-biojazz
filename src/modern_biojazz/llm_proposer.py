    ...
    def _parse_json_from_text(self, text: str) -> dict:
        ...
        if text.startswith("{") and text.endswith("}"):
            try:
                ...
                logger.debug("Failed to decode full text JSON: %s", e)
        ...
        for match in re.finditer(r"\{", text):
            ...
            try:
                ...
                logger.debug("Failed to decode JSON at pos %d: %s", start, e)
                continue
        ...
        if fenced:
            try:
                ...
                logger.debug("Failed to decode fenced JSON: %s", e)

        if not text:
            return {"actions": []}

        logger.warning("Failed to extract JSON from text: %s...", text[:50])
        return {"actions": []}
